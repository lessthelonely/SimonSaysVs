...
int hook_id; //variable whose value we get in timer_subscribe_int and use in timer_unsubscribe_int, hence why we declare it here
...
int (timer_set_frequency)(uint8_t timer, uint32_t freq) {
  ...
  uint8_t savefb=(st&0x000f); //the 4 least signficant bits are saved
  int port;
  uint8_t ctrl_word;
  if(timer==0){
...
int (timer_subscribe_int)(uint8_t *bit_no) {
  hook_id=(int)*bit_no; //Convert to the right type (by using a cast) in order to use irqsetpolicy (third argument is int)
